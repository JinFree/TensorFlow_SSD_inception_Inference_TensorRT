...
def draw_bounding_box_on_image(image,
    ...
    image = cv2.putText(image, display_str, (int(left), int(top)), cv2.FONT_HERSHEY_PLAIN, 2.0, (int(color[0]), int(color[1]), int(color[2])), thickness, cv2.LINE_AA)

    image = cv2.rectangle(image, (int(left), int(top)), (int(right), int(bottom)),(int(color[0]), int(color[1]), int(color[2])), thickness)
